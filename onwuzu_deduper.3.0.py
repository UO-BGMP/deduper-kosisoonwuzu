...
unique_four = {}    
cur_RNAME = ""   
with open(args.file, 'r') as samfile, open(args.output+"_deduped.sam", 'w') as deduped_file:
	for line in samfile:
		if line.startswith("@"):
			...
			if umi not in umi_dict:
				...
				check_four = (umi, RNAME, adj_POS, checked_FLAG)
				if check_four not in unique_four:
                    
					unique_four[umi] = (umi, RNAME, adj_POS, checked_FLAG) 
					deduped_file.write(line)
				else:
					continue
...
print("PCR duplicates have been removed from "+args.file+".")
